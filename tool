...
<body>
  <div class="app">
    <header>
      <div class="title-block">
        ...
        <div>
          ...
          <div class="subtitle">Create beautiful, printable word search puzzles with difficulty levels, teacher mode & saved setups.</div>
        </div>
      </div>
      <div class="badge">
        <span class="dot"></span>
        Live puzzle generator · PDF + solution · Teacher mode
      </div>
    </header>
    ...
    <div class="layout">
      ...
      <div class="panel">
        ...
        <div class="field">
          <label for="teacherName">Teacher name / ID</label>
          <input id="teacherName" type="text" placeholder="e.g. Miss Sara, Grade 3A" />
        </div>
        ...
        <div class="buttons">
          ...
          <button id="generateMultiBtn" class="btn-secondary">
            Teacher mode: 5 worksheets
          </button>
        </div>
        ...
        <div class="buttons" style="margin-top:4px;">
          <button id="savePuzzleBtn" class="btn-secondary">
            Save current puzzle setup
          </button>
          <button id="loadPuzzleBtn" class="btn-ghost">
            Load saved puzzles
          </button>
        </div>
      ...
      <div class="panel">
        <div class="panel-header">
          <div>
            ...
            <div class="panel-sub">Live grid + word list + saved setups</div>
          </div>
        ...
        <div class="word-list" id="saved-puzzles-list"></div>
      </div>
    </div>
  </div>
  ...
  <script>
    ...
    function renderWordList(words) {
      ...
      container.style.gridTemplateColumns = words.length >= 10 ? '1fr 1fr' : '1fr';
    ...
    document.getElementById('generateBtn').addEventListener('click', () => {
      ...
      ansBtn.classList.remove('btn-toggle-on');
    ...
    document.getElementById('downloadBtn').addEventListener('click', () => {
      ...
      function drawGrid(doc, grid, placements, highlightPlacements, startY) {
        let x, y;
        y = startY;
        const solutionMap = {};
      ...
      let y = margin + 60;
      y = drawGrid(doc, grid, placements, false, y);
      ...
      y = margin + 60;
      y = drawGrid(doc, grid, placements, true, y);
    ...
    // ---------- Teacher mode: multiple worksheets ----------
    async function generateMultipleWorksheets(count) {
      if (!window.currentWords || window.currentWords.length === 0 || !window.currentGrid) {
        alert('Please generate at least one puzzle first.');
        return;
      }
      ...
      for (let i = 0; i < count; i++) {
        ...
        const { grid, placements } = generateWordSearch(
          words,
          rows,
          cols,
          directionMode,
        ...
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(18);
        doc.text(theme + ' – Worksheet ' + (i + 1), margin, margin + 10);
        ...
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(18);
        doc.text(theme + ' – Worksheet ' + (i + 1) + ' Solution', margin, margin + 10);
      ...
      doc.save((theme || 'word-search') + '-worksheets.pdf');
      setStatus('Teacher mode: multiple worksheets PDF downloaded.', 'ok');
    }

    document.getElementById('generateMultiBtn').addEventListener('click', () => {
      generateMultipleWorksheets(5);
    });

    // ---------- Save / Load puzzles (localStorage) ----------
    function getSavedPuzzles() {
      const raw = localStorage.getItem('ws_saved_puzzles');
      if (!raw) return [];
      try {
    ...
    function setSavedPuzzles(list) {
      localStorage.setItem('ws_saved_puzzles', JSON.stringify(list));
    }

    function renderSavedPuzzles() {
      const container = document.getElementById('saved-puzzles-list');
      ...
      container.innerHTML = '<strong>Saved puzzles:</strong>';
      ...
      list.forEach((item, index) => {
        const chip = document.createElement('div');
        chip.className = 'chip';
        chip.textContent = (item.teacher || 'Unknown') + ' – ' + (item.theme || 'No theme');
        chip.style.cursor = 'pointer';
        chip.title = 'Click to load this puzzle';
        ...
        chip.addEventListener('click', () => {
          ...
          document.getElementById('words').value = (item.words || []).join('\n');
          document.getElementById('rows').value = item.rows || 12;
          document.getElementById('cols').value = item.cols || 12;
          currentDifficulty = item.difficulty || 'easy';
          document.querySelectorAll('.difficulty-btn').forEach(btn => {
            btn.classList.toggle('active', btn.getAttribute('data-level') === currentDifficulty);
          });
          updateDifficultyHint();
          setStatus('Puzzle loaded. Click "Generate puzzle" to rebuild.', 'ok');
        });
    ...
    document.getElementById('savePuzzleBtn').addEventListener('click', () => {
      ...
      renderSavedPuzzles();
      setStatus('Puzzle setup saved.', 'ok');
    });
    ...
    document.getElementById('loadPuzzleBtn').addEventListener('click', () => {
      ...
      renderSavedPuzzles();
      setStatus('Click on any saved puzzle chip to load it.', 'ok');
    });
